...
export function ShelfSection({
  label, accentColor, books, onBookClick, onAddClick, onMoreClick, preview = false, showBadge = false,
}: Props) {
        ...
        <div style={{
          ...
          {hasMore && (
            <div
              ...
              onClick={onMoreClick}
            >
              <div className="add-placeholder w-full">
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '18px', color: 'var(--add-border)' }}>+</div>
                  <div style={{ fontSize: '9px', color: 'var(--ink-light)', marginTop: '2px' }}>
                    {books.length - PREVIEW_COUNT}冊
                  </div>
                </div>
              </div>
              <p style={{ fontSize: '10px', color: 'var(--ink-light)', textAlign: 'center' }}>もっと見る</p>
            </div>
          )}
          ...
          {!hasMore && (
            ...
            >
              ...
              <p style={{ fontSize: '10px', color: 'var(--ink-light)', textAlign: 'center' }}>追加する</p>
            </div>
          )}
        </div>
      )}
